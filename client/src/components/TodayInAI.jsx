// TodayInAI — the daily "Today in AI" newsroom block. Extracted verbatim from
// BeAIReadyHome so the SAME news experience renders on both the BE AI READY door
// and the Grounded front page (a single source of truth — the two can't drift).
//
// A tiny daily newsroom: one lead story with a real photo (the lead rotates each
// day so a different category is featured), plus two shorter reads. Reads the
// three public briefings and shows nothing until at least one exists — no fake
// data, honest empty state (renders null).
...
// `to` points at the tracker — defaults to /tracker (the BE AI READY door's public
// tracker); the Grounded door passes trackerTo="/lawsuits" (its authed tracker).
export default function TodayInAI({ trackerTo = '/tracker' }) {
  ...
  const stories = [
    ...
    { kicker: 'AI Law', data: briefings.law, to: trackerTo },
    { kicker: 'Regulation', data: briefings.regulation, to: trackerTo },
