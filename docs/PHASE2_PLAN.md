...
- **2d · Onboarding UI. ✅ DONE + verified (2026-06-11).** Server: `routes/newsrooms.js` under the admin router (list/create/update newsrooms — office can't be deactivated, no delete by design; list/create users per newsroom; move users). Client: `pages/admin/NewsroomsAdmin.jsx` at `/newsrooms-admin` (AdminArea → "Newsrooms"); `ui/NewsroomSwitcher.jsx` in the AdminArea sidebar + ProductShell top bar (admins, ≥2 newsrooms) — choice persists in localStorage and rides every `apiFetch` as `X-Newsroom-Id`. Verified through the UI: created a newsroom, added its user, switched into it (empty profile, zero workflows — correctly scoped), fixtures removed.
- **2e · Enforce. ✅ DONE + verified (2026-06-11).** Migration `081_phase2_enforce_newsrooms.sql` (reversible; rollback at foot): NOT NULL on the always-written product tables; DEFAULT office + NOT NULL on the Studio/log tables; workflow slugs now **unique per newsroom** (global index replaced; create-route check scoped). Verified: two newsrooms each own a `morning-brief`.
- **2f · Node-model unification. ✅ DONE (in-repo half) + follow-up scoped.** One directory presents everything runnable: the 5 live agents + the 3 working ops-tool scaffolds (Fundraiser/Operations/Security-Audit, honestly upgraded `soon`→`partial`) + Policy Builder now link to their real workspaces from `sections.js` — 17 clickable function cards across /sections + /functions. Builder palette already carries Node blocks (node-analytics/podcasting/verifier) — Builder/Run is the spine.
  **Discovery + named follow-up:** the hosted-Node runtime is ALREADY multi-tenant (`node_<slug>_*` tables carry NOT NULL `newsroom_id`) but keys tenancy by **user id** (`tenantOf = u.id` in `grounded-node-runtime/src/server-hosted.js`) — i.e. each USER is a tenant, not each newsroom. Now that the tracker JWT carries a real `newsroom_id` (2b), unifying = change `tenantOf` to `u.newsroom_id || u.id` + bump/tag the runtime + redeploy hosted Nodes + a one-time re-key of existing `node_*` rows (user-id → office newsroom id). **Cross-repo + box data migration → do as its own deliberate change (start of Phase 3), NOT bundled into this deploy.**
