...
// Voice-check the rendered issue WITHOUT the Develop AI block (that block is
// Paul's own prose, not the writer's — including it would skew the metrics).
// Rendering to markdown keeps the [Source](url) links so the link gate is real.
const voiceOf = (issue) => checkDraft(toMarkdown(issue, ''));
...
export async function synthesizeIssue(selection, { log = console.log } = {}) {
  ...
  const voiceDraft = voiceOf(issue);
  ...
  const voiceFinal = voiceOf(issue);
